...
const nestedMenu = (menu: Array<Menu | 'divider'>, location: Location) => {
  ...
  menu.forEach((item) => {
    if (typeof item !== 'string') {
      ...
      menuItem.active =
        ((location.forceActiveMenu !== undefined && location.forceActiveMenu.includes(menuItem.pathname!)) ||
          (location.forceActiveMenu === undefined && location.pathname.includes(menuItem.pathname!)) ||
          menuItem.subMenu?.map((child) =>
              location.pathname.startsWith(child.pathname as string),
            ).some((child) => child) ||
